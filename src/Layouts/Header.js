...
export default function Header() {
  ...
  const [toggleState, setToggleState] = useState(isDarkMode);
  
  const handleToggle = () => {
    toggleTheme(); // Toggle the theme in the context
    setToggleState(!toggleState); // Toggle the local state
  };
  return (
    <div>
      <nav  className={`${isDarkMode ? "header-navbar navbar navbar-expand-lg align-items-center floating-nav navbar-light navbar-shadow container-xxl--dark" : "header-navbar navbar navbar-expand-lg align-items-center floating-nav navbar-light navbar-shadow container-xxl"} `}>
      <div className="header__button">
        <ReactSwitch onChange={handleToggle} checked={toggleState} />
        {toggleState ? 'Light Mode' : 'Dark Mode'}
      </div>
